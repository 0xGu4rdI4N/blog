...
## 2. What Does It Mean to “Model” Data? 

I earlier associated probability with the frequentist view that it is the relative occurence of an event. So are we teaching our model to just model the count of an event?

Well not exactly, what the model is learning, is actually the ability to generalize to new unseen data. As we are focusing on real data or data that has some structure or is meaningful, what we know is that this data is not just random, but has some structure or correlation between the variables of the data. Think of a real image, if one pixel's value changes to change the color, lets say, then nearby pixels also change to maintain the tag of it being a meaningful image to us. This is the very naive way to tell that our data is constrained by some rules. 

So $P(X)$ is not just a relative occurence of an event, but in higher dimensions it is like a constrain manifold. The natural training data is just a small sample of this manifold and the objective of our model is to learn this manifold using the limited data. Take protein sequences for example, what the model learn by seeing the naturally occuring sequences is the constraint manifold(constrained by various laws of physics and chemistry) and the model can generate new sequences which can actually fold and are stable in nature. As you may now think, when we speak of Data Generating Process, the nature has some fundamental rules governing the generation and this is what makes the data lie on a constrain manifold. 

To summarize, $P(X)$ is a function in higher dimensions of the constraints. Its high will be at a data point which is more likely to have been generated by nature or in other words, which is meaningful to us. 
